...
export interface ResolvePickerKindInput {
  ...
  /**
   * The active conversation has received/submitted at least one user message.
   * Blank conversations created from side-nav launchers are still in the
   * pre-run selection state, so they should behave like home for picker
   * purposes.
   */
 ...
 *
 *  - Home or a blank pre-run conversation (local or cloud): the AgentProfile
 *    picker, which starts a new conversation / activates the default (#3727,
 *    cloud via #15060). When no profiles exist yet, fall back — cloud → model,
 *    local → LLM-profile (cloud has no home LLM-profile activate path).
