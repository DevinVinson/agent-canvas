...
## Automation Architecture & Failure Diagnostics

- The automation backend is the `openhands-automation` PyPI package (currently v1.0.0a3), started via `uvx openhands-automation` in `scripts/dev-with-automation.mjs`. Source code is not in this repo; it lives in the installed site-packages (e.g. `~/.cache/uv/archive-v0/.../openhands/automation/`).
- **Root cause of local automation failures**: The automation backend's `execution.py` hardcodes `DEFAULT_WORK_DIR = "/workspace/project"` — a cloud-only path that doesn't exist locally. The `LocalExecutionService.execute()` method calls `_run_setup()` which runs `setup.sh` inside this nonexistent directory, producing `[Errno 2] No such file or directory: '/workspace/project'`. This is a backend-side bug that needs to be fixed in the `openhands-automation` package.
- **Secondary failure: Python version mismatch**: Even if the work dir existed, `setup.sh` runs `uv venv` which picks the first `python3` on PATH. On macOS the system Python is 3.9.x but `openhands-sdk` requires >= 3.12. The venv is created with 3.9 and `uv pip install openhands-sdk` fails with a cryptic resolver error. Added a `checkPythonVersion()` prerequisite check to `dev-with-automation.mjs` to catch this early.
- **Automation run workspace layout**: Each automation run gets a directory at `~/.openhands/agent-canvas/workspaces/automation-runs/{run_id}/`. The tarball for the automation code is extracted here.
- **Error detail display**: Failed runs have an `error_detail` field in the API response (`AutomationRun` type). Added a collapsible error panel to `activity-log-item.tsx` so users can see the error without hitting the API directly. i18n key: `AUTOMATIONS$DETAIL$ERROR_DETAIL_LABEL`.
