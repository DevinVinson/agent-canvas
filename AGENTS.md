...
- Local UI add-ons are frontend-only and owned by `agent-canvas`: valid add-ons live under `addons/<addon-id>/.openhands/addon.json`, are discovered by `npm run make-addons` / `scripts/generate-addons-registry.mjs`, and generate `src/addons/registry.generated.ts`. Routed add-ons appear in the main sidebar and mount only under `/addons/:addonId/*`. Style-only shell add-ons may set `frontend.route=false`, `sidebar.visible=false`, and `styling.appCss` to contribute top-level CSS without adding a route; their CSS should scope through `[data-agent-server-ui][data-agent-canvas-addons~="<addon-id>"]`. Do not add agent-server routes, backend modules, install/update flows, or arbitrary top-level route injection for add-ons unless a later product decision explicitly broadens the v1 contract.
